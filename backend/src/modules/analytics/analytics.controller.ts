...
export class AnalyticsController {
  ...
  getMyAnalytics(@CurrentUser() user: any, @Query() query: AnalyticsQueryDto) {
    return this.analyticsService.getVendorAnalytics(user.vendorId, query);
  }
  ...
  getRevenue(@CurrentUser() user: any, @Query() query: AnalyticsQueryDto) {
    return this.analyticsService.getRevenueReport(user.vendorId, query);
  }
  ...
  getTopVendors(@Query('category') category?: string, @Query('limit') limit = 10) {
