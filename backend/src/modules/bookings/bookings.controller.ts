...
import { CreateBookingDto, UpdateBookingDto, BookingStatus } from './dto/booking.dto';
...
export class BookingsController {
  ...
  getMyBookings(@CurrentUser() user: any, @Query('status') status?: BookingStatus) {
  ...
  getVendorBookings(@CurrentUser() user: any, @Query('status') status?: BookingStatus) {
    return this.bookingsService.findForVendor(user.vendorId, status);
  }
  ...
  @UseGuards(RolesGuard)
  @Roles('VENDOR')
  @ApiOperation({ summary: '[Vendor] Confirm a pending booking (PENDING → CONFIRMED)' })
  confirm(@CurrentUser() user: any, @Param('id') id: string) {
    return this.bookingsService.confirm(user.vendorId, id);
  }

  @Put(':id/complete')
  @ApiOperation({ summary: 'Mark booking as completed (CONFIRMED → COMPLETED)' })
