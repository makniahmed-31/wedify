...
import { Star, MapPin, Phone, Globe, MessageCircle, Clock, Shield, Crown, Check, ChevronLeft, Calendar } from "lucide-react";
import { Header } from "@/components/layout/header";
import { Footer } from "@/components/layout/footer";
import { getVendorBySlug, getReviewsByVendor, MOCK_VENDORS } from "@/lib/mock-data";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: vendor.seo?.metaTitle ?? `${vendor.businessName} — ${vendor.category.name} in ${vendor.city.name}`,
    description: vendor.seo?.metaDescription ?? vendor.description.substring(0, 160),
    keywords: vendor.seo?.keywords ?? [vendor.businessName, vendor.category.name, vendor.city.name, "Tunisia"],
    openGraph: {
    ...
    <>
      ...
      <main className="flex-1">
        ...
        <div className="relative h-72 md:h-96 overflow-hidden">
          <Image src={vendor.coverImage} alt={vendor.businessName} fill className="object-cover" priority />
          <div className="absolute inset-0 gradient-hero" />
          <div className="absolute inset-0 flex items-end">
            <div className="mx-auto max-w-7xl w-full px-4 sm:px-6 lg:px-8 pb-8">
              <Link href={`/${vendor.category.slug}`} className="inline-flex items-center gap-1.5 text-white/70 text-sm mb-3 hover:text-white">
              ...
              <div className="flex items-end justify-between gap-4">
                <div>
                  ...
                  {vendor.tagline && <p className="text-white/80 mt-1">{vendor.tagline}</p>}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div className="rounded-xl border bg-card p-4 text-center">
                  <div className="text-xl font-bold">{vendor.yearsInBusiness ?? "N/A"}</div>
                ...
                <div className="rounded-xl border bg-card p-4 text-center">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">{vendor.responseTime ?? "Quick response"}</p>
                </div>
                <div className="rounded-xl border bg-card p-4 text-center">
                  <div className="text-xl font-bold">{vendor.languages.length}</div>
                  <p className="text-xs text-muted-foreground mt-1">Languages spoken</p>
                </div>
              </div>

              {/* About */}
              <section>
                <h2 className="text-xl font-bold mb-3">About {vendor.businessName}</h2>
                <p className="text-muted-foreground leading-relaxed">{vendor.description}</p>
                <div className="flex flex-wrap gap-2 mt-4">
                  {vendor.languages.map((lang) => (
                    <span key={lang} className="rounded-full border px-3 py-1 text-xs">{lang}</span>
                  ))}
                </div>
              </section>
                ...
                <section>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {vendor.gallery.map((img, i) => (
                      <div key={i} className={`relative overflow-hidden rounded-xl ${i === 0 ? "row-span-2 col-span-2 sm:col-span-1" : ""}`} style={{ aspectRatio: i === 0 ? "1" : "4/3" }}>
                        <Image src={img} alt={`${vendor.businessName} photo ${i + 1}`} fill className="object-cover hover:scale-105 transition-transform duration-300" sizes="(max-width: 768px) 50vw, 33vw" />
                      </div>
                    ))}
                  </div>
                </section>
              ...
              {vendor.services.length > 0 && (
                <section>
                  ...
                  <div className="space-y-3">
                    {vendor.services.filter((s) => s.isActive).map((service) => (
                      <div key={service.id} className="rounded-xl border bg-card p-4 flex items-center justify-between gap-4">
                        <div>
                          <h3 className="font-semibold">{service.name}</h3>
                          <p className="text-sm text-muted-foreground mt-1">{service.description}</p>
                          {service.duration && <p className="text-xs text-muted-foreground mt-1">Duration: {service.duration}</p>}
                        </div>
                        <div className="text-right shrink-0">
                          <p className="font-bold">{service.price.toLocaleString()} {CURRENCY_SYMBOL}</p>
                          <p className="text-xs text-muted-foreground capitalize">{service.priceUnit.replace(/_/g, " ").toLowerCase()}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {vendor.packages.length > 0 && (
                <section>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {vendor.packages.filter((p) => p.isActive).map((pkg) => (
                      <div
                        key={pkg.id}
                        className={`rounded-2xl border p-5 relative ${pkg.isPopular ? "border-primary ring-1 ring-primary/30 shadow-gold" : ""}`}
                      >
                        {pkg.isPopular && (
                    ...
                    <div key={review.id} className="rounded-xl border bg-card p-5">
                      ...
                      {review.vendorReply && (
                        <div className="mt-3 rounded-lg bg-muted p-3">
                          <p className="text-xs font-semibold mb-1">Response from {vendor.businessName}</p>
                          <p className="text-xs text-muted-foreground">{review.vendorReply}</p>
                        </div>
                      )}
                    </div>
                  ))}
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-2xl border bg-card p-5 sticky top-20">
                <div className="text-center mb-4">
                  {vendor.minPrice ? (
                    <>
                      ...
                      <p className="text-3xl font-bold">{vendor.minPrice.toLocaleString()} <span className="text-lg">{CURRENCY_SYMBOL}</span></p>
                    </>
                  ) : (
                ...
                <div className="space-y-3 pt-3 border-t">
                  <div className="flex items-center gap-3 text-sm">
                    ...
                    {vendor.city.name}{vendor.address ? `, ${vendor.address}` : ""}
                  </div>
                  <div className="flex items-center gap-3 text-sm">
                  ...
                  {vendor.website && (
                    <div className="flex items-center gap-3 text-sm">
                      ...
                      <a href={vendor.website} target="_blank" rel="noopener noreferrer" className="hover:text-primary truncate">
              ...
              <div className="rounded-2xl border bg-card p-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {vendor.languages.map((lang) => (
                    <span key={lang} className="rounded-full border px-2.5 py-0.5 text-xs">{lang}</span>
                  ))}
                </div>
              </div>
            </div>
