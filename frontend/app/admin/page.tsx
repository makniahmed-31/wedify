...
import { TrendingUp, Users, Store, CreditCard, DollarSign, AlertCircle } from "lucide-react";

const MRR_DATA = [
  { month: "Jan", mrr: 18500 }, { month: "Feb", mrr: 22400 }, { month: "Mar", mrr: 26800 },
  { month: "Apr", mrr: 31200 }, { month: "May", mrr: 34500 }, { month: "Jun", mrr: 39100 },
  { month: "Jul", mrr: 43800 }, { month: "Aug", mrr: 48200 }, { month: "Sep", mrr: 52400 },
  { month: "Oct", mrr: 57900 }, { month: "Nov", mrr: 62100 }, { month: "Dec", mrr: 68400 },
];
...
const PENDING_REVIEWS = [
  { vendor: "Elegance Hall", reviewer: "Sarah & Ahmed", rating: 5, date: "1h ago" },
  { vendor: "Dream Photography", reviewer: "Fatima & Khalil", rating: 4, date: "3h ago" },
  { vendor: "Sweet Moments", reviewer: "Rim & Yassine", rating: 5, date: "5h ago" },
];
  ...
  return (
    <div className="space-y-8 max-w-7xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "MRR", value: "68,400 TND", change: "+10.2%", icon: DollarSign },
          { label: "ARR", value: "820,800 TND", change: "+10.2%", icon: TrendingUp },
          { label: "Total Vendors", value: "1,850", change: "+42 this month", icon: Store },
          { label: "Active Subscriptions", value: "1,734", change: "93.7% paid", icon: CreditCard },
        ].map((kpi) => (
      ...
      <div className="rounded-2xl border bg-card p-6">
        ...
        <p className="text-sm text-muted-foreground mb-6">2025 growth trajectory</p>
        <ResponsiveContainer width="100%" height={260}>
          <AreaChart data={MRR_DATA}>
            ...
            <Tooltip formatter={(v) => [`${Number(v).toLocaleString()} TND`, "MRR"]} contentStyle={{ borderRadius: "12px", border: "1px solid hsl(var(--border))", backgroundColor: "hsl(var(--card))" }} />
              ...
              <div key={tier.plan}>
                <div className="flex items-center justify-between mb-1.5 text-sm">
                  ...
                  <div className="flex gap-4 text-muted-foreground">
                    ...
                    <span className="font-medium text-foreground">{tier.revenue.toLocaleString()} TND/mo</span>
                  </div>
                </div>
            ...
            {PENDING_REVIEWS.map((r, i) => (
              <div key={i} className="flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button className="rounded-lg bg-green-500/10 px-2.5 py-1 text-xs font-medium text-green-600 hover:bg-green-500/20">Approve</button>
                  <button className="rounded-lg bg-red-500/10 px-2.5 py-1 text-xs font-medium text-red-500 hover:bg-red-500/20">Reject</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="rounded-2xl border bg-card p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <a href="/admin/vendors" className="text-sm text-primary hover:underline">View all</a>
        </div>
          ...
          <tbody>
            {[
              ...
              <tr key={v.name} className="border-b last:border-0 hover:bg-muted/20">
                ...
                <td className="py-3">
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${v.plan === "PRO" ? "bg-blue-500/10 text-blue-600" : "bg-muted text-muted-foreground"}`}>{v.plan}</span>
                </td>
                <td className="py-3">
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${v.status === "ACTIVE" ? "bg-green-500/10 text-green-600" : "bg-yellow-500/10 text-yellow-600"}`}>{v.status}</span>
                </td>
              </tr>
            ))}
          </tbody>
