        ...
        {[
          {
            ...
            desc: "Désactive l'accès public au site",
          },
          {
            key: "registrationOpen",
            label: "Inscriptions ouvertes",
            desc: "Autoriser les nouvelles inscriptions",
          },
          {
            key: "autoApproveVendors",
            label: "Approbation automatique",
            desc: "Approuver les prestataires sans validation manuelle",
          },
        ].map(({ key, label, desc }) => (
          <div
            key={key}
          ...
          >
            ...
            >
              <span
                className={`pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow-lg transition-transform ${settings[key as keyof typeof settings] ? "translate-x-5" : "translate-x-0"}`}
              />
            </button>
          </div>
        ))}
        ...
        <div className="flex items-center justify-between gap-4 py-2">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Recevoir des alertes pour les nouvelles inscriptions et
              signalements
            </p>
          </div>
          <button
          ...
          >
            <span
              className={`pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow-lg transition-transform ${settings.emailNotifications ? "translate-x-5" : "translate-x-0"}`}
            />
          </button>
        </div>
      </section>
