...
const FEATURED_SLOTS = [
  {
    slot: 1,
    vendorId: "elegance-hall-tunis",
    vendorName: "Elegance Hall Tunis",
    category: "Salles de mariage",
    active: true,
  },
  {
    slot: 2,
    vendorId: "chateau-des-roses-hammamet",
    vendorName: "Château des Roses",
    category: "Salles de mariage",
    active: true,
  },
  {
    slot: 3,
    vendorId: "villa-jasmin-la-marsa",
    vendorName: "Villa Jasmin",
    category: "Salles de mariage",
    active: true,
  },
  {
    slot: 4,
    vendorId: "band-el-andalus-tunis",
    vendorName: "Band El Andalus",
...
export default function AdminHomepagePage() {
  ...
  function toggleSlot(slot: number) {
    setSlots((s) =>
      s.map((x) => (x.slot === slot ? { ...x, active: !x.active } : x)),
    );
  }
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Gérer le contenu affiché sur la page principale
          </p>
        </div>
      ...
      <div className="rounded-lg border bg-card p-6 space-y-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="ml-auto text-xs text-muted-foreground">
            {slots.filter((s) => s.active).length}/4 actifs
          </span>
        </div>
        <div className="space-y-2">
          {slots.map((s) => (
            <div
              key={s.slot}
              className={`flex items-center justify-between gap-4 rounded-md border p-3 transition-colors ${s.active ? "bg-primary/5 border-primary/20" : "opacity-60"}`}
            >
