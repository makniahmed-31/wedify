...
export default function AdminUsersPage() {
  ...
  const filtered = users.filter((u) => {
    ...
    return !search || u.email.toLowerCase().includes(q) ||
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {loading ? "Chargement..." : `${users.length} utilisateurs inscrits`}
          </p>
        </div>
        <button onClick={load} disabled={loading}
          ...
          {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : null} Actualiser
        </button>
      </div>
                      ...
                      <div className="flex items-center gap-2">
                        <div className="h-8 w-8 rounded-full bg-muted flex items-center justify-center shrink-0">
                          {u.role === "ADMIN" ? <Shield className="h-4 w-4 text-red-500" /> :
                            u.role === "VENDOR" ? <Crown className="h-4 w-4 text-blue-500" /> :
