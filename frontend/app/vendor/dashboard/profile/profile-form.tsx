...
import { Save, Camera, Globe, Phone, MessageCircle, Mail } from "lucide-react";
...
function GalleryEditor({
  ...
  function updateUrl(i: number, val: string) {
    const next = gallery.map((u, idx) => (idx === i ? val : u));
    onChange(next);
  }
  function addRow() {
    onChange([...gallery, ""]);
...
export default function ProfileForm({ initialData }: { initialData: Partial<ProfileForm> | null }) {
  const [form, setForm] = useState<ProfileForm>(() => ({
    ...
    ...(initialData ? {
      ...
      gallery: (initialData.gallery as string[] | undefined)?.length ? initialData.gallery as string[] : [""],
    } : {}),
  }));
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setError("");
    try {
      const res = await apiFetch("/api/v1/vendors/profile/me", {
        method: "PUT",
  ...
  return (
    <div className="space-y-8">
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <div className="rounded-lg border bg-card p-6">
          ...
          <div className="relative h-40 rounded-md bg-muted overflow-hidden mb-4 flex items-center justify-center">
            <div className="text-center text-muted-foreground">
              ...
              <p className="text-sm">Cover image upload coming soon</p>
            </div>
          </div>
        </div>
        ...
        {/* Pricing */}
        <div className="rounded-lg border bg-card p-6 space-y-4">
          <h2 className="font-semibold">Price Range (TND)</h2>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium mb-1.5">Starting From</label>
              <input
                type="number"
                min={0}
                value={form.minPrice}
            ...
            <div>
              <label className="block text-sm font-medium mb-1.5">Up To</label>
              <input
                type="number"
                min={0}
                value={form.maxPrice}
        ...
        <div className="rounded-lg border bg-card p-6 space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            Add public image URLs to showcase your work (Google Drive shared link, Imgur, Cloudinary, etc.).
          </p>
          <GalleryEditor
        ...
        <button
          type="submit"
          disabled={saving}
          className="flex items-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-semibold text-white shadow-gold hover:opacity-90 transition-opacity disabled:opacity-60"
        >
