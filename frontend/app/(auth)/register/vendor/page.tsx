...
export default function VendorRegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (step === "business") { setStep("contact"); return; }
    if (step === "contact") { setStep("plan"); return; }
    setLoading(true);
    ...
    return (
      <div className="text-center py-8">
        ...
        <p className="text-muted-foreground mb-6 max-w-sm mx-auto">
          Welcome to Wedify! Your vendor account has been created. You can now set up your profile and start receiving bookings.
        </p>
        <Link href="/dashboard" className="inline-flex items-center rounded-full gradient-gold px-6 py-3 text-sm font-semibold text-white shadow-gold">
          Go to Dashboard
        </Link>
      </div>
    );
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-1">Start receiving wedding bookings on Wedify</p>
      </div>
        ...
        {STEPS.map((s, i) => (
          <div key={s.id} className="flex items-center gap-2 flex-1">
            <div className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold ${
              ...
              {STEPS.indexOf(STEPS.find((x) => x.id === step)!) > i ? <Check className="h-3.5 w-3.5" /> : i + 1}
            </div>
          ...
          <>
            ...
            <div>
              ...
              <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
                rows={3} className="w-full rounded-xl border bg-background px-4 py-2.5 text-sm outline-none focus:ring-1 focus:ring-primary resize-none"
          ...
          <div className="space-y-3">
            {SUBSCRIPTION_PLANS.map((plan) => (
              <button
                ...
                className={`w-full rounded-2xl border p-4 text-left transition-all ${form.plan === plan.name ? "border-primary ring-1 ring-primary/30 shadow-gold" : "hover:border-primary/30"}`}
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-bold">{plan.label}</span>
                      {plan.isPopular && <span className="rounded-full gradient-gold px-2 py-0.5 text-xs text-white">Popular</span>}
                    </div>
